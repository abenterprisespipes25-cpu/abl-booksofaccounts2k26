...
/** Professional CDB Recap Export using ExcelJS */
export async function exportRecapCDBExcel(opts: {
  ...
  const borderMedTopBottom = { top: medSide, bottom: medSide, left: thinSide, right: thinSide };

  // Sheet 1: SUNDRIES RECAP
  const ws1 = workbook.addWorksheet("Sundries-Recap", {
    pageSetup: {
      paperSize: 5, orientation: "portrait",
  ...
  ws1.addRow([companyName]).getCell(1).font = fontArial10Bold;
  ws1.addRow(["RECAPITULATION OF SUNDRY ACCOUNTS — Cash Disbursements Book"]);
  ws1.addRow([`FOR THE MONTH OF ${monthYear.toUpperCase()}`]);
  ws1.addRow([]);
  const headRow1 = ws1.addRow(["S  U  N  D  R  I  E  S", "Debit", "Credit"]);
  headRow1.eachCell(c => { c.font = fontArial10Bold; c.border = borderThin; c.alignment = { horizontal: "center" }; });

  let totalDr = 0; let totalCr = 0;
  recapSundries.forEach(s => {
    totalDr += s.dr; totalCr += s.cr;
    const r = ws1.addRow([s.account, s.dr || "", s.cr || ""]);
    r.getCell(1).font = fontArial9;
  ...
  const totRow = ws1.addRow(["GRAND TOTAL", totalDr || "", totalCr || ""]);
  totRow.eachCell((c, i) => {
    c.font = fontArial10Bold;
    c.border = borderMedTopBottom;
  ...
  ws2.addRow([companyName]).getCell(1).font = fontArial10Bold;
  ws2.addRow(["RECAPITULATION OF BANK ACCOUNTS — Cash Disbursements Book"]);
  ws2.addRow([`FOR THE MONTH OF ${monthYear.toUpperCase()}`]);
  ws2.addRow([]);
  const headRow2 = ws2.addRow(["FUND", "AMOUNT"]);
  headRow2.eachCell(c => { c.font = fontArial10Bold; c.border = borderThin; c.alignment = { horizontal: "center" }; });

  let totalBank = 0;
  recapFunds.forEach(f => {
    totalBank += f.amount;
    const r = ws2.addRow([f.fund, f.amount || ""]);
    r.getCell(1).font = fontArial9;
    r.getCell(1).border = borderThin;
    r.getCell(2).numFmt = '#,##0.00';
    r.getCell(2).border = borderThin;
  ...
  const totRow2 = ws2.addRow(["TOTAL", totalBank || ""]);
  totRow2.eachCell((c, i) => {
    c.font = fontArial10Bold;
    c.border = borderMedTopBottom;
